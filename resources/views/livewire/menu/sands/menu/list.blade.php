<div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
    @if ($isModalOpen)
    @include('livewire.menu.sands.menu.create')
    @endif
    ...
    <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
        ...
        <tbody class=" w-full divide-y divide-gray-100 border-t border-gray-100">
            @foreach ($menus as $menu)
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                <td class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                    <div class="relative h-10 w-10">
                        <img class="h-full w-full rounded-full object-cover object-center" src="{{ $menu->image }}" alt="" />
                        <span class="absolute right-0 bottom-0 h-2 w-2 rounded-full {{ $menu->status === 1 ? 'bg-green-400' : 'bg-red-400' }} ring ring-white"></span>
                    </div>
                    <div class="text-sm">
                        <div class="font-medium text-gray-700">{{ Str::limit($menu->main_title, 30, '...') }}</div>
                        <div class="text-gray-400">{{ Str::limit($menu->description, 25, '...') }}</div>
                    </div>
                </td>
                <td class="px-6 py-4">
                    @if ($menu->status === 1)
                ...
                <td class="px-6 py-4">{{ $menu->categories->description }}</td>
